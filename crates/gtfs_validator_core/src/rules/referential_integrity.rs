...
impl Validator for ReferentialIntegrityValidator {
    ...
    fn validate(&self, feed: &GtfsFeed, notices: &mut NoticeContainer) {
        let stop_ids: HashSet<gtfs_guru_model::StringId> = feed
            .stops
            .rows
            .iter()
            ...
            .filter(|id| id.0 != 0)
            .collect();
        let trip_ids: HashSet<gtfs_guru_model::StringId> = feed
            .trips
            .rows
            .iter()
        ...
        let route_ids: HashSet<gtfs_guru_model::StringId> = feed
            ...
            .filter(|id| id.0 != 0)
            .collect();
        let network_ids: HashSet<gtfs_guru_model::StringId> = feed
            .networks
            .as_ref()
            .map(|table| {
                table
            ...
            .unwrap_or_default();
        let area_ids: HashSet<gtfs_guru_model::StringId> = feed
            .areas
            .as_ref()
            .map(|table| {
                table
            ...
            .unwrap_or_default();
        let fare_media_ids: HashSet<gtfs_guru_model::StringId> = feed
            .fare_media
            .as_ref()
            .map(|table| {
                table
            ...
            .unwrap_or_default();
        let fare_product_ids: HashSet<gtfs_guru_model::StringId> = feed
            .fare_products
            .as_ref()
            .map(|table| {
                table
            ...
            .unwrap_or_default();
        let rider_category_ids: HashSet<gtfs_guru_model::StringId> = feed
            .rider_categories
            .as_ref()
            .map(|table| {
                table
            ...
            .unwrap_or_default();
        let timeframe_group_ids: HashSet<gtfs_guru_model::StringId> = feed
            .timeframes
            .as_ref()
            .map(|table| {
                table
            ...
            .unwrap_or_default();
        let fare_leg_group_ids: HashSet<gtfs_guru_model::StringId> = feed
            .fare_leg_rules
            .as_ref()
            .map(|table| {
                table
            ...
            .unwrap_or_default();
        let location_group_ids: HashSet<gtfs_guru_model::StringId> = feed
            .location_groups
            .as_ref()
            .map(|table| {
                table
            ...
            .unwrap_or_default();
        let booking_rule_ids: HashSet<gtfs_guru_model::StringId> = feed
            .booking_rules
            .as_ref()
            .map(|table| {
                table
        ...
        if let Some(fare_leg_rules) = &feed.fare_leg_rules {
            for (index, rule) in fare_leg_rules.rows.iter().enumerate() {
                ...
                if let Some(timeframe_id) = rule.to_timeframe_group_id.filter(|id| id.0 != 0) {
                    if !timeframe_group_ids.contains(&timeframe_id) {
                        let timeframe_value = feed.pool.resolve(timeframe_id);
                        notices.push(missing_ref_notice(
                            CODE_FOREIGN_KEY_VIOLATION,
