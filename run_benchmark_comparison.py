...
for i, test_path in enumerate(test_cases):
    ...
    java_cmd = [java_bin, "-jar", str(validator_jar), "--input", str(test_path), "--output_base", str(case_output_dir / "java")]
    java_success = False
    ...
    try:
        ...
        if proc.returncode == 0:
            rust_success = True
            report_path = case_output_dir / "rust" / "report.json"
    ...
    results.append({
        "name": test_name,
        "java": {"success": java_success, "count": java_notices_count, "duration": java_duration, "codes": java_codes},
        "rust": {"success": rust_success, "count": rust_notices_count, "duration": rust_duration, "codes": rust_codes},
